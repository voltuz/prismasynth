...
- Auto-fix path: `TimebaseWarningDialog → RemuxProgressDialog → core.timebase_remuxer.RemuxJob` (QThread; sequential `ffmpeg -y -nostdin -v error -i <input> -c copy -video_track_timescale <N> <output>` with cancel-kills-proc + per-source `source_done` signals). MainWindow probes each fixed file and routes through the existing `_apply_relink_results` — same machinery as Tools → Relink…, so reader/proxy/thumbnail/clip-clamp logic isn't duplicated.
