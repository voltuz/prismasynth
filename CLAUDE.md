...
- **`scripts/perf_benchmark.py`** — perf timing harness for decode / thumbnail / scene-detection / export. Hand-rolled FFmpeg, NOT through `Exporter` — use `system_check.py` for correctness. ~110s.
- **`scripts/cut_inspect.py`** — mpv-embedded Qt GUI launchable from Tools → Cut Inspect (or standalone) for inspecting ±2 frames around exported cut boundaries. The Tools launcher spawns it as a separate Python process so its mpv can't conflict with the preview's.
- **`scripts/export_diag.py`** — runs a single segment through multiple ffmpeg config variants (two-stage seek, single seek, CPU decode, NVENC, …) and reports first-frame hashes.
...
- **`scripts/crop_brightness_check.py`** — runs the pre-fix (no HDR handling) and post-fix (HDR-aware) crop-export filter chains on the same source and `signalstats`-compares the per-frame Y means. PASS criterion: HDR sources show >30 Y-mean brightness uplift, SDR sources show <5 |delta|. Run after any change to `core/crop_exporter.py`'s filter chain.
...
**Menu bar:** File / Edit / Timeline / Tools / View (UI Scale). Built in `MainWindow._setup_menus`.
...
- `VideoSource` — immutable per-source metadata; see `core/video_source.py`.
- Project files (`.psynth`) are JSON. See `core/project.py::save_project` for the schema.
...
- `PreviewWidget` creates an mpv instance embedded in a QWidget via window handle (`wid`). Property config (hardware decode, accurate frame seek, demuxer cache) lives in `PreviewWidget.__init__`.
...
- `File → Keyboard Shortcuts…` opens `KeyboardShortcutsDialog` (in `ui/shortcuts_dialog.py`). Uniqueness is enforced — conflicts pop a warning. Persistence: `QSettings` keys under `shortcuts/<id>`. On startup, ShortcutManager detects collisions (e.g. a new release adds a default that clashes with a user override) and clears one side so the unique-key invariant holds.
...
- **Quick-cut:** Right-click during playhead drag splits at the playhead — no mode switch needed.
...
- The timeline strip reserves space below each clip for up to three group chips (name + color + W3C-luminance-based readable text); `+N` overlay on the last visible chip when a clip belongs to more groups than fit.
...
- `ProjectVersionStore(project_path)` is the single API surface. MainWindow rebinds it on save / load via `_ensure_version_store()`.
- **Valid triggers:** `autosave`, `manual`, `pre_detect_cuts`, `pre_multi_delete`, `pre_group_delete`, `pre_source_removal`, `pre_restore`. Anything else logs a warning and `create()` returns None.
- **Pre-op snapshot points** (in `main_window.py`): `_do_remove_source`, `_on_delete` / `_on_ripple_delete` when >5 clips selected, post-detect before `replace_detected`, the `PeoplePanel.group_delete_confirmed` signal (emitted after the user confirms the delete prompt but *before* the timeline mutation), and `_on_restore_version`. `_take_pre_op_snapshot` flushes dirty state to disk first so the snapshot reflects the working state.
- **Filename shape:** `YYYY-MM-DD_HH-MM-SS[_trigger][_label-slug].psynth`; same-second collisions get `_2`, `_3`, … suffix.
- **Retention (`prune()`):** newest 50 kept unconditionally; older entries thinned to one-per-hour / -day / -week buckets (see `_HOURLY_WINDOW` / `_DAILY_WINDOW`). Runs after every `create()`.
- **Self-healing list (`list_versions`)** reconciles `index.json` against `*.psynth` on disk every call: dropped files removed, orphan files re-derived via `_derive_entry()`. `create()` reads the manifest via `_read_manifest()` directly to avoid the orphan-redupe trap (see Key Conventions).
- **Restore flow:** `VersionsDialog.restore_requested(filename)` → MainWindow `_on_restore_version` → flush dirty + take `pre_restore` snapshot → `_load_from(version_path, project_path_override=original_path)` → mark dirty. The override keeps the live `_project_path` pointed at the user's real `.psynth` while loading version contents in-memory.
...
- **Action enable/disable:** `_refresh_version_actions()` greys out `File → Project Versions…` and `File → Snapshot Project Now…` until the project has a path on disk. `_refresh_snapshot_action()` does the same for `Tools → Snapshot Frame` and additionally requires the playhead to be over a non-gap clip.
...
- `core.frame_snapshot.snapshot_frame_to_png()` runs a one-shot ffmpeg pass that reuses the exporter's `(frame_num - 0.5) / fps` half-frame seek margin (same IEEE-754 rationale as `Exporter._frame_to_seek_ts`), so the saved PNG matches the preview pixel-for-pixel. Best-effort: failures log and surface "Snapshot failed — see logs" in the status bar.
- **Output:** `<project_stem>_snapshots/{source_stem}_f{source_frame_num}_{HHMMSS}.png`. Source frame derived via `TimelineModel.timeline_frame_to_source_frame(playhead)`.
- **Disabled state:** action greys out when the project has no path yet, or the playhead is over a gap. `_refresh_snapshot_action()` recomputes on every `_on_playhead_frame` and on project lifecycle hooks.
...
- **Cache Manager…** — `ui/cache_manager_dialog.py`. Per-category disk usage with Clear / Refresh / Open Folder. Releases proxy mmap locks via `proxy_manager.close_all()` before clearing the proxies category — Windows otherwise refuses to delete in-use mmap'd files.
...
The `Export XML…` / `Export OTIO…` menu entries route into this same unified dialog at `tab=3` / `tab=4`.
...
**Routing** is in `MainWindow._run_export`: video → `_export_video_parallel`; `audio_mode="standalone"` → `_export_audio_only`; `"both"` → video + `_extract_audio_sidecar`; xml/otio → `core.xml_exporter` / `core.otio_exporter`.
...
- **No segment coalescing in the parallel path** — timeline order wins over the small perf delta from coalescing same-source clips.
...
- **HDR metadata safety:** Filter chain prepends `setparams=color_primaries=bt2020:color_trc=smpte2084:colorspace=bt2020nc` so `tonemap_opencl` has consistent signalling even when the seek lands on a mid-GOP P/B frame, and APPENDS `setparams=color_primaries=bt709:color_trc=bt709:colorspace=bt709` after the tonemap so the encoded stream's primaries tag doesn't leak as `bt2020`.
- **Dolby Vision sources:** Currently exported through the static HDR10 tonemap chain. DV's per-scene dynamic metadata (RPU NAL units) is ignored. Output is darker / less saturated than a DV-aware player would produce — most visible on UHD Blu-ray rips (Profiles 5, 7, 8). Future work: investigate `libplacebo`'s `dovi` filter or `dolbyvision=mode=convert` as a pre-tonemap stage.
- **Chapter / data / subtitle track drop:** All ffmpeg export invocations (per-segment, mux, concat, image-sequence) include `-map_chapters -1 -dn -sn`. Without these, Blu-ray rips' chapter tracks (stored as `bin_data` streams in the source MOV) propagate through the concat `-c copy` step and inherit the source movie's full-runtime duration — players then report a 5-second crop export as ~1:25h long.
...
- **Spine layout:** one `<asset-clip>` per real clip. `offset` = cumulative timeline position; `start` = source in-point (with the NTSC nudge); `include_gaps=True` emits `<gap>` elements. Source URI lives on a `<media-rep kind="original-media">` child of the (shared, per-source) `<asset>`.
...
- **Persistent coordinator thread + 6-worker `ThreadPoolExecutor`** for the session. `notify_clips_changed()` rebuilds the clip lookup without destroying the cache or pool.
- **Pause-on-pan, NOT pause-on-scrub:** Middle-mouse pan of the timeline pauses the coordinator (rapid viewport reprioritization is wasteful) — `pan_started`/`pan_ended` signals on `TimelineStrip` connect to `_thumbnail_cache.pause()`/`resume()`. Scrubbing the playhead does NOT pause the coordinator — with baked thumbnails on disk its work is cheap JPEG decode that doesn't contend with mpv.
- **Memory-only cache** (`_mem_cache`): QImage at 192x108 keyed by `{source_id}_{frame_num}`. `.copy()` on QImage to prevent use-after-free across thread boundaries.
- **GPU isolation:** Thumbnails use CPU-only ffmpeg (no `-hwaccel cuda`) to leave NVDEC exclusively for mpv scrubbing.
- **Cache Thumbnails dialog** (`ui.cache_thumbnails_dialog.CacheThumbnailsDialog`) bakes the on-disk cache for the first and last frame of every non-gap clip (or only clips inside the in/out range). Backed by `thumbnail_cache.BulkCacheJob`. The per-source worker-count cap (see Key Conventions) avoids a historical PyAV native segfault on 4+ concurrent containers per source.
...
- **Setup** — `scripts/setup_omnishotcut.py` installs Python 3.10 via `uv`, creates the sidecar venv, downloads the model checkpoint, writes a `.prismasynth_ready` sentinel as the LAST step. Re-runnable with `--repair`.
- **Sidecar** (`scripts/omnishotcut_sidecar.py`) loads the model ONCE per Detect Cuts run (5-15s) and processes all segments sequentially. JSON-line stdio + raw frame bytes.
- **Runner** (`core.omnishotcut_runner.OmnishotcutRunner`) cancels via `CTRL_BREAK_EVENT` then `proc.kill()`. `is_setup_complete()` checks sentinel + sidecar python + `third_party/OmniShotCut/`.
...
- `core.video_source.is_seek_safe(time_base_num, time_base_den, fps)` — module-level safety predicate. `utils/ffprobe.py::probe_video` reads `stream.time_base` onto `VideoInfo`; legacy projects re-probe on first load.
- `MainWindow._warn_unsafe_timebases` fires from import, drag-drop, project load, and relink. Lists unsafe sources via `ui.timebase_warning_dialog.TimebaseWarningDialog`.
- Auto-fix path: `TimebaseWarningDialog → RemuxProgressDialog → core.timebase_remuxer.RemuxJob` (QThread; sequential `ffmpeg -c:v copy -video_track_timescale <N>`). Only the first audio stream is mapped — matches `core.exporter`, discards Bluray-rip commentary tracks. MainWindow probes each fixed file and routes through the existing `_apply_relink_results`.
- Audio mode (QSettings key `timebase_autofix/audio_mode`): `keep` (`-c:a copy`, default) / `reencode_same` (PCM 16-bit @ 48 kHz, layout preserved) / `stereo` (PCM 2.0, ffmpeg's default downmix matrix so 5.1 condenses rather than drops). Flags built in `RemuxJob._audio_args`.
- Per-source progress: `RemuxJob` parses `out_time_us=` lines from ffmpeg's `-progress pipe:1` stdout while a daemon thread drains stderr (mirrors `core.exporter._drain` — avoids the OS-pipe-fills-and-deadlocks failure).
- Output path: `<basename>_fixed.mov` next to the original, MOV regardless of input container (Resolve compatibility). Stream copy preserves frame count + ordering, so `Clip.source_in/source_out` indices line up 1:1 in the fixed file. Sibling `<basename>.mkv` is auto-detected and preferred as the remux input. Target timescale = the FCPXML denominator the exporter writes (24000 for 23.976, etc.).
...
**Paint order** is defined in `paintEvent`.
