...
class RemoveSourceDialog(QDialog):
    def __init__(self, source_name: str, clip_count: int, parent=None):
        super().__init__(parent)
        self.setWindowTitle("Remove Source")
        ...
        msg = QLabel(
            ...
            f"clip{'s' if clip_count != 1 else ''} on the timeline.<br><br>"
        ...
        keep_btn = QPushButton(
            f"Remove source — keep {clip_count} clip{'s' if clip_count != 1 else ''} on timeline"
        )
        keep_btn.setToolTip(
        ...
        with_btn = QPushButton(
            f"Remove source AND its {clip_count} clip{'s' if clip_count != 1 else ''}"
        )
        with_btn.setStyleSheet("color: #e87a75;")
        with_btn.setToolTip("Permanently removes both the source and every clip referencing it.")
